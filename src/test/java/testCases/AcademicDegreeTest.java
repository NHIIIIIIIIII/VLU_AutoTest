...
public class AcademicDegreeTest extends BaseTest {
     ...
     * @since 06/03/2025
     */
    @Test(dataProvider = "updateInformationData", dataProviderClass = JsonReader.class)

    public void testEditAD(String name,  int order) {
        // Delay để chờ trước khi thực hiện thao tác (tuỳ chỉnh nếu cần)
        sleep(10);

        // Thực hiện thêm học hàm học vị
        academicDMPage.editADDetails(name, order);

        // Bạn có thể thêm assertion để xác nhận kết quả nếu cần
//         Assert.assertTrue(driver.getPageSource().contains(name), "Academic Degree '" + name + "' was not edited successfully");
    }
